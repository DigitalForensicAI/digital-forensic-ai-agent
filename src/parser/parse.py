...
from .canonical import CanonicalEvent, CanonicalCase, normalize_timestamp
from .db import insert_events, get_timeline
...
def parse_csv_log(file_path: str, default_source: str = "sysmon", default_session: str = "s1") -> List[CanonicalEvent]:
    ...
    with open(file_path, "r", encoding="utf-8", errors="replace") as f:
        ...
        if any(h in sample_lower for h in ["timestamp", "actor", "event_type", "object", "command"]):
            has_header = True
        ...
        for row in reader:
            ...
            def get_val(name: str, fallback_idx: int = -1, default: str = "") -> str:
            ...
            raw_line = ",".join(row)
            artifact_id = get_val("artifact_id", -1, f"evt_{idx:05d}")
            ts = get_val("timestamp", 0)
            actor = get_val("actor", 1)
            event_type = get_val("event_type", 2)
            obj = get_val("object", 3)
            cmd = get_val("command", 4)
            src_ip = get_val("src_ip", 5)
            dst_ip = get_val("dst_ip", 6)
            source = get_val("source", -1, default_source)
