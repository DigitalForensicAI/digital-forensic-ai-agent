...
DEFAULT_RULES_PATH = Path(__file__).parent / "mitre_map.json"
...
def evaluate_mitre(event: Dict[str, Any], techniques: List[Dict[str, Any]]) -> List[Dict[str, str]]:
    ...
    for tech in techniques:
        cond = tech.get("conditions", {})
        matched = True

        req_types = cond.get("event_types")
        if req_types and event_type not in [t.lower() for t in req_types]:
            matched = False

        obj_contains = cond.get("object_contains")
        if obj_contains and not any(sub.lower() in obj for sub in obj_contains):
            matched = False

        cmd_contains = cond.get("command_contains")
...
def correlate_from_file(input_path: str, rules_path: Optional[str] = None) -> Dict[str, Any]:
...
def main():
    ...
    result = correlate_from_file(args.input_path, rules_path=args.rules)
